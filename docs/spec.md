...
│       ├── validate.py       # 検証スクリプト
│       ├── common.py         # ビルド共通ユーティリティ（パス管理・コマンド実行）
│       ├── config.py         # 設定管理（YAML ロード・スキーマ検証）
│       ├── ff_utils.py       # FontForge 補助ユーティリティ
│       └── ft_utils.py       # fontTools 補助ユーティリティ
├── build/                    # 中間生成物
├── dist/                     # 最終成果物
...
- `metrics`: `upm=2048`、`half_width=1233`、`full_width=2466`、`ascender=1556`、`descender=-492`、`line_gap=0`、`typo_ascender=1901`、`typo_descender=-483`、`typo_line_gap=0`、`win_ascent=1901`、`win_descent=483`、`x_height=1120`、`cap_height=1493`、`is_fixed_pitch=true`、`bizud_visual_scale=0.94`
...
2. `bizud_visual_scale` (0.94) を乗じた追加縮小を適用し、仮想ボディ内のマージンを確保
   - 計算式：`scale = (full_width / scaled_width) * bizud_visual_scale`
3. 縦横均等スケーリングを適用（アスペクト比を維持）
4. グリフを `full_width` の枠内で中央揃え
5. `advance width` を `full_width` に強制設定
...
| nameID | 内容 | 値 |
|---|---|---|
| 0 | Copyright | "Copyright 2026 HA-Gothick Contributors. Hack (c) Christopher Simpkins. BIZ UDGothic (c) Morisawa Inc." |
...
| 11 | Vendor URL | "<https://github.com/cmt1910/ha-gothick>" |
...
`ttfautohint` は `stem-width-mode=nnn`、`increase-x-height=14`、`no-info`、`fallback-script=latn` を指定して `build/optimized-{weight}.ttf` から `build/hinted-{weight}.ttf` を生成する。
...
**ヒンティング失敗時の動作:** `ttfautohint` が失敗した場合、警告を表示して最適化済みフォント（`optimized-{weight}.ttf`）をそのままヒント済みファイルとしてコピーし、後続の日本語ヒント除去・メタデータ再適用処理を継続する。

`build.py` には `--skip-hinting` オプションが存在し、これを指定すると ttfautohint をスキップして最適化済みフォントをそのまま使用する。
...
この処理では、対象グリフが複合グリフである場合に備えて、その構成要素として参照されるコンポーネントグリフも再帰的に収集してヒントを削除する。

- 入力: `build/hinted-{weight}.ttf`
- 出力: `build/hint_stripped-{weight}.ttf`

##### 7.4 最終メタデータ再適用

`patch_tables.py` を再実行して以下を確定する:

- `maxp.maxSizeOfInstructions` の再計算（日本語ヒント除去後の整合）
- name / OS/2 / hhea テーブルの最終確定

- 入力: `build/hint_stripped-{weight}.ttf`
- 出力: `build/finalized-{weight}.ttf`

##### 7.5 サブセット化（任意）

配布サイズ削減のためにグリフを絞る場合:

`pyftsubset` では `build/finalized-{weight}.ttf` を入力とし、Latin、罫線、CJK、全角・半角記号、Powerline、Nerd Fonts PUA、Supplementary PUA を残したサブセットを `build/subset-{weight}.ttf` として出力する。

#### 出力

- アウトライン最適化済みフォント (`build/optimized-{weight}.ttf`)
- ヒンティング適用済みフォント (`build/hinted-{weight}.ttf`)
- 日本語ヒンティング除去済みフォント (`build/hint_stripped-{weight}.ttf`)
- メタデータ最終確定済みフォント (`build/finalized-{weight}.ttf`)

...
| 役割 | フォント名 | ライセンス | 提供ウェイト | UPM |
|---|---|---|---|---|
| 欧文 | Hack | Modified SIL OFL 1.1 + Bitstream Vera License | Regular, Bold, Italic, Bold Italic | 2048 |
...
1. **ウェイト対応の確認** — Hack (Regular/Bold/Italic/BoldItalic) と BIZ UDゴシック (Regular/Bold) の対応関係を確認する。BIZ UDゴシックにイタリック体がないため、Italic / BoldItalic ウェイトでは日本語部分に Regular / Bold を流用する
...
| 4 | Full Name | "HA-Gothick Regular" |
| 5 | Version | "Version 1.0.0" |
| 6 | PostScript Name | "HA-Gothick-Regular" |
...
```python
os2 = font["OS/2"]
os2.sTypoAscender  = 1556
os2.sTypoDescender = -492
os2.sTypoLineGap   = 410
os2.usWinAscent    = 1901
os2.usWinDescent   = 483
os2.sxHeight       = 1120
os2.sCapHeight     = 1493
os2.xAvgCharWidth  = 1233          # half_width
...
```python
head = font["head"]
# macStyle: 0 = Regular, 1 = Bold, 2 = Italic, 3 = Bold Italic
...
3. **重複ポイント除去** — 同一座標上の冗長なポイントを削除
4. **極値ポイントの追加** — カーブの極値位置にポイントを配置（ヒンティング品質向上）
...
| `OS/2.xAvgCharWidth` | `1233` (half_width) |
...
Options:
  --weight <name>   単一ウェイトのみビルド (Regular|Bold|Italic|BoldItalic)
...
├── HA-Gothick-Bold.ttf
├── HA-Gothick-Italic.ttf
├── HA-Gothick-BoldItalic.ttf
├── LICENSE.txt
└── README.md
```

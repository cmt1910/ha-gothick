...
- Python パッケージ管理: `uv`
- フォント加工: `FontForge`
- 自動ヒンティング: `ttfautohint` を欧文中心に適用し、日本語グリフのヒンティングは後段で除去
- 実行環境: `Docker` (`linux/amd64`)
- 見た目調整: 英字との厳密な中心一致より、日本語の仮想ボディ感を優先して BIZ UDGothic の縦位置を固定
...
設定値は [config/config.yaml](config/config.yaml) で管理します。既定では `metrics.y_offset: 0` を使い、日本語グリフを不自然に下げずに元の重心を維持します。

...
def compute_y_offset(font, config: BuildConfig, visual_scale: float) -> int:
    ...
    return round(target_center - (source_center * visual_scale))
...
def main(argv: list[str] | None = None) -> int:
    ...
    visual_scale = config.metrics.bizud_visual_scale
    ...
    for codepoint in iter_range_list(FULL_WIDTH_RANGES):
        if not glyph_exists(font, codepoint):
            continue
        fit_glyph(font[codepoint], full_width, y_offset, visual_scale)
        adjusted_full += 1
    ...
    for codepoint in iter_range_list(HALF_WIDTH_RANGES):
        if not glyph_exists(font, codepoint):
            continue
        fit_glyph(font[codepoint], half_width, y_offset, visual_scale)
        adjusted_half += 1
